...
def build_loc_cost_tn(n_qubits, x, contracted_envs, cost_tn):
    r"""
    ...
    if x == 0:
        loc_cost_tn = gates_to_opt & contracted_envs["R"][f"R{x}"]
    elif x == n_qubits - 1:
        loc_cost_tn = contracted_envs["L"][f"L{x}"] & gates_to_opt
    else:
        loc_cost_tn = (
            contracted_envs["L"][f"L{x}"] & gates_to_opt & contracted_envs["R"][f"R{x}"]
        )
...
def optimize_single_gate_update(
    n_qubits, cost_tn, rtol, n_sweeps_max, transfer_structure, contracted_envs
):
    r"""
        ...
        for sweep_instr in instr:
            for x in sweep_instr[1]:
                ...
                loc_cost_tn, gate_to_opt_tags = build_loc_cost_tn(
                    n_qubits,
                    x=x,
                    contracted_envs=contracted_envs,
                    cost_tn=cost_tn,
                )
                ...
                for tag in gate_to_opt_tags:
                    ...
                    contracted_envs = update_dict_contr_envs(
                        mode=sweep_instr[0],
                        gate_tensors=[original_gate_tens],
                        cost_tn=cost_tn,
                        transfer_structure=transfer_structure,
                        contracted_envs=contracted_envs,
                    )
...
def transpile_mpo_to_circuit(
    ref_mpo, depth, rtol, n_sweeps_max, *, param_scaling=1e-1, closed=False, rng=None
):
    r"""
