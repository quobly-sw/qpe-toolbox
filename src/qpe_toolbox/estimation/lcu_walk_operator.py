...
def _gates_llxHl(hamiltonian, l_term):
    ...
    phys_reg = list(range(m_L, m_L + hamiltonian.num_qubits))
...
def _build_Hl_mpo(hamiltonian, l_term):
    ...
    if l_term >= len(hamiltonian.terms):
        return qtn.MPO_identity(hamiltonian.num_qubits)
    ...
    for i in range(hamiltonian.num_qubits):
        if i in qubits:
            ...
            mat = qu.identity(2)
        if (i == 0) or (i == (hamiltonian.num_qubits - 1)):
...
def run_qpe_lcu_walk_operator(
    ...
    regs = _get_registers_qpe_lcu(H.num_qubits, m_L, m_ph)
...
def qpe_first_stage_walk(
    ...
    circ = qtn.CircuitMPS(
        m_ph + m_L + H.num_qubits, psi0=psi_init, max_bond=max_bond, cutoff=cutoff
    )
...
def _get_registers_qpe_lcu(num_qubits, m_L, m_ph):
    """
    ...
    regs["phys"] = tuple(range(m_ph + m_L, m_ph + m_L + num_qubits))
    return regs
