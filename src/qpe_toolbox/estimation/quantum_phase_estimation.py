...
def qpe_energy(
    ...
    *,
    ...
    Notes
    -----
    - The phase register is automatically determined as ``initial_circ.N - hamiltonian.num_qubits``.
    ...
    n_phase_bits = initial_circ.N - hamiltonian.num_qubits
...
def qpe_sample(
    ...
    *,
    ...
    if write_gates:
        ...
        filename = f"QPE_ttr{trotter_order}{n_steps}steps_{hamiltonian.num_qubits}qubits_{n_phase_bits}phbits"
        if run_simulation:
...
def qpe_first_stage(
    ...
    * Optional Trotterization for approximate time evolution.
    ...
    Notes
    -----
    - The phase register size is inferred from ``initial_circ.N - hamiltonian.num_qubits``.
    ...
    data_reg = [n_phase_bits + i for i in range(hamiltonian.num_qubits)]
    phase_reg = list(range(n_phase_bits))
