...
#
# The former discussion was fully dedicated to the `CircuitMPS` class from $\texttt{quimb}$. Nevertheless, there exists another MPS class that tries to leverage the non-locality of long-range circuits: `PermMPS`. The goal of this class can be understood by diving a bit into the procedure for applying a long-range gate: it requires permuting the two involved qubits through the 1-dimensional layout until they are nearest-neighbors, followed by the local application of the two-body gate. While `CircuitMPS` would restore the initial layout after the gate application, `PermMPS` leaves this new layout and tracks the intermediate trajectories of the qubits. Obviously, this class may yield advantageous wall-clock time measures when the number of gates is sparse. Opposed to that, our circuit instances are not sparse enough to detect a clear advantage. This can be seen for $D=1$ in the right column (random entangling pattern). Despite these results, we expect that a study with varying `ent_prob` and `ent_range` in the function `generate_rand_circuit` would define a regime of dominance of `PermMPS` over `CircuitMPS` for sparse circuits (low `ent_prob`, medium-low `ent_range`).
