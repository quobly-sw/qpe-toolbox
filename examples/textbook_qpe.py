...
data_reg = list(range(n_phase_bits, n_phase_bits + num_qubits))
print(
...
fig.suptitle(f"1D Heisenberg with {num_qubits} spins")
...
for num_qubits in tqdm.tqdm(nqb_list):
    h_spin = heisenberg_hamiltonian(num_qubits)
...
# %%
for ind, num_qubits in enumerate(nqb_list):
    plt.plot(ms, res["energies"][ind], "-o", label=f"$n_{{qb}}=${num_qubits}")
plt.ylabel("Energy")
