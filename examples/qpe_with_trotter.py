...
#
# We run and measure the energy for several QPE circuits varying the number of phase qubits and Trotter steps. We also record the gate count and simulation time, and compare two modes of circuit simulation available with $\texttt{quimb}$: the `Circuit` mode and the `CircuitMPS` mode.
#
...
# %% [markdown]
# * The `CircuitMPS` mode is much more efficient than the `Circuit` mode (actually, the timing depends on the contraction order found by the optimizer, see our notebook on [Hyperoptimization](./hyperoptimization.ipynb)). The computation time is directly correlated with the number of entangling gates, which grows exponentially with the number of phase qubits.
#
