...
n_shots = qpe.EXACT  # exact computation (no sampling)
...
# m = 0
phi_0 = qpe.rpe_get_hadamard_output(H, psi0, 0, qpe.EXACT, n_shots)
theta_0 = phi_0

m = 1
phi_1 = qpe.rpe_get_hadamard_output(H, psi0, m, qpe.EXACT, n_shots)
...
theta_list = qpe.robust_phase_estimation(
    H, psi0, epsilon, sign_E0, qpe.EXACT, n_shots, verbosity=1
)
...
for n_shots in n_shot_list:
    theta_list = qpe.robust_phase_estimation(
        H, psi0, epsilon, sign_E0, qpe.EXACT, n_shots
    )
    plt.semilogy(
        [qpe.rpe_distance(theta, E0) for theta in theta_list[1:]],
        "-o",
...
for n_shots in n_shot_list:
    theta_list = qpe.robust_phase_estimation(
        H_H2, psi0_H2, epsilon, sign_E0, qpe.EXACT, n_shots
    )
    plt.semilogy(
